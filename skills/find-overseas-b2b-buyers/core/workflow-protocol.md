...
For calibration, stop discovery near three times the requested lead count. If fewer candidates survive, expand the best-yield matrix cells or run one additional query family before lowering any threshold. Cache normalized domains and decisive evidence so subsequent runs do not repeat unchanged work.
